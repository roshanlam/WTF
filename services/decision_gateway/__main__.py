...
class DecisionGateway:
    ...
    def _validate_llm_result(self, llm_result: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if llm_result["food_available"]:
            ...
            missing_fields = [
                f for f in event_fields if f not in llm_result or not llm_result[f]
            ]
            if missing_fields:
    ...
    def _create_decision_payload(
        ...
        payload = {
            ...
            "club_name": llm_result.get(
                "club_name", original_data.get("club_name", "Unknown")
            ),
    ...
    def process_tweet(self, row_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            decision_payload = self._create_decision_payload(
                llm_result, row_data, latency, ground_truth
            )
            ...
            logger.error(
                f"[GATEWAY] Exception processing tweet from {club_name}: {str(e)}"
            )
            return {
                "decision": "ERROR",
                "status": "exception",
    ...
    def get_notifications_from_batch(self, batch_results: Dict[str, Any]) -> list:
        ...
        logger.info(
            f"[GATEWAY] Extracted {len(notifications)} notifications from batch"
        )
