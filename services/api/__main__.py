...
class UnifiedPipeline:
    ...
    def _create_event_from_payload(
        self, payload: dict, original_data: dict
    ) -> FreeFoodEvent:
        """
    ...
    def process_single_tweet(self, row_data: dict) -> dict:
        ...
        try:
            ...
            if decision["decision"] == "SEND_NOTIFICATION":
                ...
                logger.info("   Consumer services will now process this event")
                logger.info("   → MQ Consumer: Will log event details")
                logger.info("   → Notification Service: Will send emails")
                ...
                logger.info(
                    f"  Reason: {decision.get('payload', {}).get('processing_metadata', {}).get('llm_reason', 'N/A')}"
                )
    ...
    def _print_summary(self, results: list):
        ...
        if self.stats["total_processed"] > 0:
            ...
            if self.stats["food_detected"] > 0:
                detection_rate = (
                    self.stats["food_detected"] / self.stats["total_processed"] * 100
                )
        ...
        if published:
            ...
            for i, event in enumerate(published, 1):
                logger.info(
                    f"  {i}. {event.get('event_title')} - {event.get('club_name')}"
                )
        ...
        logger.info("\n💡 Next Steps:")
        logger.info(
            "   1. Ensure MQ Consumer is running: python -m services.mq_consumer"
        )
        logger.info(
            "   2. Ensure Notification Service is running: python -m services.notification_service"
        )
...
def main():
    ...
    pipeline = UnifiedPipeline(model_path=args.model, rate_limit_delay=args.delay)
